...
def main(action,architecture,application,version,build,recurse,long,hidestatus,hidesupported,diff,debug):
    if action == "conform":
        reportconform.main(architecture,application,version,build,recurse,long)
    elif action == "conformdist":
        reportconformdist.main(architecture,
                               application,
                               version,
                               build,
                               recurse,
        ...
        reporthtml.main(architecture,hidestatus,hidesupported,debug)
    elif action == "text":
        reporttext.main(architecture,application,version,build,recurse,long,diff)
    else:
        print("%s is not a valid action." % action)
        print("Must be one of ['conform', 'conformdist', 'html', 'text'].")
